...
-- Earlier product migrations created several of these relations user-scoped.
-- CREATE TABLE IF NOT EXISTS preserves those definitions, so reconcile every
-- pre-existing relation before indexes, constraints, or RLS reference its
-- tenant key.  A row without one deterministic active membership fails closed.
DO $$
DECLARE
  v_table text;
...
BEGIN
  FOREACH v_table IN ARRAY ARRAY[
    'company_knowledge_documents', 'marketing_campaigns', 'finance_invoices',
    'workforce_members', 'website_funnels', 'commerce_products',
    'commerce_orders', 'notifications', 'provider_subscriptions',
    'webhook_endpoints'
  ] LOOP
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = v_table AND column_name = 'workspace_id')
       AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = v_table AND column_name = 'id') THEN
      EXECUTE format('SELECT EXISTS (SELECT 1 FROM public.%I WHERE workspace_id IS NULL)', v_table) INTO v_unresolved;
      IF v_unresolved THEN RAISE EXCEPTION 'Backfill assertion failed: %.workspace_id is unresolved.', v_table; END IF;
