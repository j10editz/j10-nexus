-- ============================================================================
-- J10 NEXUS: Complete Standalone Staging Database Setup
-- Target: j10-nexus-staging (Supabase)
-- Includes: Base Schema + Omnichannel + Telegram Business Secretary Mode + Cron
-- ============================================================================
...
-- Drop existing functions to allow signature/parameter name updates cleanly
...
DROP FUNCTION IF EXISTS public.consume_telegram_binding_token(text, text, text) CASCADE;
DROP FUNCTION IF EXISTS public.consume_telegram_business_session(text, text, text) CASCADE;
DROP FUNCTION IF EXISTS public.ingest_telegram_update_transactional CASCADE;
DROP FUNCTION IF EXISTS public.claim_telegram_ai_jobs CASCADE;
DROP FUNCTION IF EXISTS public.renew_telegram_ai_job_lease CASCADE;
DROP FUNCTION IF EXISTS public.acquire_telegram_worker_lock CASCADE;
DROP FUNCTION IF EXISTS public.release_telegram_worker_lock CASCADE;
DROP FUNCTION IF EXISTS public.complete_telegram_ai_job CASCADE;
DROP FUNCTION IF EXISTS public.preview_telegram_business_deletion CASCADE;
DROP FUNCTION IF EXISTS public.create_telegram_deletion_intent CASCADE;
DROP FUNCTION IF EXISTS public.execute_telegram_scoped_deletion CASCADE;
DROP FUNCTION IF EXISTS public.trigger_telegram_ai_worker_cron CASCADE;


-- ----------------------------------------------------------------------------
-- 1. BASE WORKSPACE & AUTH SCHEMA
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.workspaces (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL DEFAULT 'Staging Workspace',
  slug TEXT UNIQUE NOT NULL DEFAULT ('ws-' || substr(gen_random_uuid()::text, 1, 8)),
  owner_user_id UUID,
...
CREATE TABLE IF NOT EXISTS public.workspace_memberships (
  ...
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now(),
  CONSTRAINT uq_staging_ws_member UNIQUE (workspace_id, user_id)
);
...
-- ----------------------------------------------------------------------------
-- 3. CRM CONTACTS & OMNICHANNEL INBOX
-- ----------------------------------------------------------------------------
...
AS $$
DECLARE
  v_row public.telegram_binding_tokens;
BEGIN
  SELECT * INTO v_row
...
-- >>> START TELEGRAM BUSINESS MIGRATION <<<
...
-- >>> START TELEGRAM CRON RECONCILIATION MIGRATION <<<
