...
SELECT jsonb_build_object(
  ...
  'policies', COALESCE((SELECT jsonb_agg(jsonb_build_object('table',c.relname,'name',pol.polname,'command',pol.polcmd,'roles',CASE WHEN 0=ANY(pol.polroles) THEN ARRAY['PUBLIC']::name[] ELSE ARRAY(SELECT rolname FROM pg_roles WHERE oid=ANY(pol.polroles) ORDER BY rolname) END,'using',pg_get_expr(pol.polqual,pol.polrelid),'check',pg_get_expr(pol.polwithcheck,pol.polrelid)) ORDER BY c.relname,pol.polname) FROM pg_policy pol JOIN pg_class c ON c.oid=pol.polrelid JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public'),'[]'::jsonb),
